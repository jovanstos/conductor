...
function loadStoredOllamaUrl(): string {
  try {
    return (
      localStorage.getItem("conductor_ollamaUrl") ?? "http://localhost:11434"
    );
  } catch {
...
export const useSettingsStore = create<SettingsStore>()((set, get) => ({
  ...
  setDefaultModel: (model) => {
    try {
      localStorage.setItem("conductor_defaultModel", JSON.stringify(model));
    } catch {
      /* ignore */
    }
  ...
  setOllamaUrl: (url) => {
    try {
      localStorage.setItem("conductor_ollamaUrl", url);
    } catch {
      /* ignore */
    }
    set({ ollamaUrl: url });
