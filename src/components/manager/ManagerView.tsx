...
import { Plus, Target, Trash2, ChevronRight, Briefcase } from "lucide-react";
...
export default function ManagerView() {
  ...
  return (
    <div className="h-full flex overflow-hidden">
          ...
          {missions.length === 0 ? (
            ...
            missions.map((m) => {
              ...
              const isRunning =
                ...
                status === "escalating" ||
                status === "briefing";
              const activeGoals = m.goals.filter(
                (g) => g.status === "active",
              ).length;
              const doneGoals = m.goals.filter(
              ...
              return (
                <div key={m.id} className="relative group px-2">
                  {deletingId === m.id ? (
                    <div
                      className="mx-1 my-1 px-3 py-2.5 rounded-xl border"
                      style={{
                    ...
                    >
                      ...
                      <div className="shrink-0 mt-0.5">
                        {isRunning ? (
                          <span
                            className="pulse-accent w-2.5 h-2.5 rounded-full inline-block"
                            style={{
                              background:
                                status === "briefing"
                                  ? "var(--c-amber)"
      ...
      {showNewModal && (
        <CreateMissionModal onClose={() => setShowNewModal(false)} />
      )}
    </div>
  );
}
    ...
    <div className="h-full flex flex-col items-center justify-center gap-4 text-center p-8">
      ...
      <div className="max-w-sm">
        ...
        <p
          ...
          style={{ color: "var(--c-text-3)" }}
        >
          Deploy a Manager Agent that runs autonomously toward your goals —
          dispatching specialist workers, reviewing their output, and escalating
