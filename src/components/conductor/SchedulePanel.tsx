  ...
  return (
    <div className="p-4 space-y-4">
      <div className="flex items-center gap-2 mb-2">
        <CalendarClock
          size={16}
          style={{ color: enabled ? "var(--c-amber)" : "var(--c-text-3)" }}
        />
        <span
      ...
      <div>
        <label
          className="block text-xs uppercase tracking-widest font-semibold mb-1.5"
        ...
        <input
          ...
          onChange={(e) => {
            ...
            saveSchedule({ task: e.target.value });
          }}
        />
      </div>
          ...
          {INTERVAL_OPTIONS.map((opt) => (
            <button
              ...
              onClick={() => {
                ...
                saveSchedule({ interval: opt.value });
              }}
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            <input
              type="number"
              min={1}
              max={interval === "minutes" ? 59 : 23}
              value={intervalValue}
              onChange={(e) => {
                setIntervalValue(Number(e.target.value));
                saveSchedule({ intervalValue: Number(e.target.value) });
              }}
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              saveSchedule({ time: e.target.value });
            }}
      ...
      >
        Scheduled runs trigger while the app is open. For persistent scheduling,
        use an OS task scheduler or system service.
      </p>
    </div>
  );
}
...
function ToggleSwitch({
  ...
  const accentDim =
    color === "amber" ? "var(--c-amber-dim)" : "var(--c-accent-dim)";
  return (
    <button
      onClick={() => onChange(!value)}
      className="relative w-12 h-6 rounded-full transition-all"
      style={{
        background: value ? accentDim : "var(--c-card)",
        border: `1px solid ${value ? accentColor : "var(--c-border)"}`,
      }}
    >
      <span
